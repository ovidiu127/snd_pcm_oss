...
static int snd_pcm_plugin_alloc(struct snd_pcm_plugin *plugin, 
...
int snd_pcm_plug_alloc(struct snd_pcm_substream *plug, snd_pcm_uframes_t 
frames)
{
	int err;
	...
	if (snd_pcm_plug_stream(plug) == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		while (plugin->next) {
			if (plugin->dst_frames)
				frames = plugin->dst_frames(plugin, 
frames);
...
snd_pcm_sframes_t snd_pcm_plugin_client_channels(struct snd_pcm_plugin 
*plugin,
				       snd_pcm_uframes_t frames,
			 ...
			 struct snd_pcm_plugin **ret)
{
	...
	plugin->src_width = 
snd_pcm_format_physical_width(src_format->format);
	snd_BUG_ON(plugin->src_width <= 0);
	plugin->dst_format = *dst_format;
	plugin->dst_width = 
snd_pcm_format_physical_width(dst_format->format);
	snd_BUG_ON(plugin->dst_width <= 0);
	if (plugin->stream == SNDRV_PCM_STREAM_PLAYBACK)
		channels = src_format->channels;
	else
		channels = dst_format->channels;
	plugin->buf_channels = kcalloc(channels, 
sizeof(*plugin->buf_channels), GFP_KERNEL);
...
*format_mask)
{
	...
	if (snd_pcm_format_linear(format)) {
		...
		snd_pcm_format_t best_format = (__force 
snd_pcm_format_t)-1;
		...
		case SNDRV_PCM_FORMAT_MU_LAW:
			for (i = 0; i < ARRAY_SIZE(preferred_formats); 
++i) {
				snd_pcm_format_t format1 = 
preferred_formats[i];
				if (snd_mask_test(format_mask, (__force 
int)format1))
					return format1;
			}
			fallthrough;
...
SNDRV_PCM_ACCESS_RW_NONINTERLEAVED);
	...
	if (srcformat.channels > dstformat.channels) {
		...
		err = snd_pcm_plugin_build_route(plug, &srcformat, 
&tmpformat, &plugin);
		pdprintf("channels reduction: src=%i, dst=%i returns 
...
&tmpformat,
		...
		tmpformat.rate = dstformat.rate;
        	err = snd_pcm_plugin_build_rate(plug,
        					&srcformat, &tmpformat,
						&plugin);
		pdprintf("rate down resampling: src=%i, dst=%i returns 
...
srcformat.format, tmpformat.format, err);
	...
	if (srcformat.channels < dstformat.channels) {
		...
		err = snd_pcm_plugin_build_route(plug, &srcformat, 
&tmpformat, &plugin);
		pdprintf("channels extension: src=%i, dst=%i returns 
%i\n", srcformat.channels, tmpformat.channels, err);
		if (err < 0)
			return err;
		err = snd_pcm_plugin_append(plugin);
...
snd_pcm_sframes_t snd_pcm_plug_client_channels_buf(struct 
...
snd_pcm_sframes_t snd_pcm_plug_write_transfer(struct snd_pcm_substream 
*plug, struct snd_pcm_plugin_channel *src_channels, snd_pcm_uframes_t 
size)
{
	...
	while (plugin) {
		...
		if (next) {
			...
			if (plugin->dst_frames) {
				frames1 = plugin->dst_frames(plugin, 
frames);
				if (frames1 <= 0)
					return frames1;
			}
			err = next->client_channels(next, frames1, 
&dst_channels);
			if (err < 0)
				return err;
			if (err != frames1) {
				...
				if (plugin->src_frames) {
					frames = 
plugin->src_frames(plugin, frames1);
					if (frames <= 0)
						return frames;
				}
			}
		...
		frames = plugin->transfer(plugin, src_channels, 
dst_channels, frames);
		if (frames < 0)
			return frames;
...
snd_pcm_sframes_t snd_pcm_plug_read_transfer(struct snd_pcm_substream 
*plug, struct snd_pcm_plugin_channel *dst_channels_final, 
snd_pcm_uframes_t size)
{
	...
	while (plugin && frames > 0) {
		next = plugin->next;
		if (next) {
			err = plugin->client_channels(plugin, frames, 
&dst_channels);
			if (err < 0)
				return err;
			frames = err;
			 ...
			 size_t samples, snd_pcm_format_t format)
{
	...
	if (!dst_area->addr)
		return 0;
	dst = dst_area->addr + (dst_area->first + dst_area->step * 
dst_offset) / 8;
...
int snd_pcm_area_copy(const struct snd_pcm_channel_area *src_area, size_t 
src_offset,
		      const struct snd_pcm_channel_area *dst_area, size_t 
dst_offset,
		      size_t samples, snd_pcm_format_t format)
{
	/* FIXME: sub byte resolution and odd dst_offset */
	...
	src = src_area->addr + (src_area->first + src_area->step * 
src_offset) / 8;
	if (!src_area->addr)
