...
MODULE_AUTHOR("Jaroslav Kysela <perex@perex.cz>, Abramo Bagnara 
<abramo@alsa-project.org>");
MODULE_DESCRIPTION("PCM OSS emulation for ALSA.");
MODULE_LICENSE("GPL");
module_param_array(dsp_map, int, NULL, 0444);
MODULE_PARM_DESC(dsp_map, "PCM device number assigned to 1st OSS 
device.");
module_param_array(adsp_map, int, NULL, 0444);
MODULE_PARM_DESC(adsp_map, "PCM device number assigned to 2nd OSS 
device.");
...
static int snd_pcm_oss_get_channels(struct snd_pcm_oss_file 
*pcm_oss_file);
 ...
 */
static int snd_interval_refine_min(struct snd_interval *i, unsigned int 
min, int openmin)
{
	int changed = 0;
	if (i->min < min) {
...
static int snd_interval_refine_max(struct snd_interval *i, unsigned int 
max, int openmax)
{
	int changed = 0;
	if (i->max > max) {
...
static int snd_interval_refine_set(struct snd_interval *i, unsigned int 
val)
{
			   ...
			   snd_pcm_hw_param_t var, int *dir)
{
	...
	if (hw_is_interval(var)) {
		const struct snd_interval *i = hw_param_interval_c(params, 
var);
		if (dir)
			*dir = i->openmin;
				 ...
				 int dir)
{
	...
	if (hw_is_mask(var))
		...
		changed = 
snd_interval_refine_min(hw_param_interval(params, var),
						  val, open);
	else
		return -EINVAL;
	if (changed > 0) {
...
static int snd_pcm_hw_param_min(struct snd_pcm_substream *pcm,
				...
				int *dir)
{
	int changed = _snd_pcm_hw_param_min(params, var, val, dir ? *dir : 
0);
	if (changed < 0)
		return changed;
	if (params->rmask) {
				 ...
				 int dir)
{
	...
	if (hw_is_mask(var)) {
		if (val == 0 && open) {
			...
			changed = 
snd_mask_refine_max(hw_param_mask(params, var),
						      val - !!open);
	} else if (hw_is_interval(var))
		changed = 
snd_interval_refine_max(hw_param_interval(params, var),
						  val, open);
	else
		return -EINVAL;
	if (changed > 0) {
...
static int snd_pcm_hw_param_max(struct snd_pcm_substream *pcm,
				...
				int *dir)
{
	int changed = _snd_pcm_hw_param_max(params, var, val, dir ? *dir : 
0);
	if (changed < 0)
		return changed;
	if (params->rmask) {
 ...
 * @dir: pointer to the direction (-1,0,1) or NULL
 *
 * Inside configuration space defined by PARAMS set PAR to the available 
value
...
static int snd_pcm_hw_param_near(struct snd_pcm_substream *pcm,
				 struct snd_pcm_hw_params *params,
				 snd_pcm_hw_param_t var, unsigned int 
best,
				 int *dir)
{
	...
	*save = *params;
	...
	if (min >= 0) {
		...
		*params1 = *save;
		max = snd_pcm_hw_param_max(pcm, params1, var, max, 
&maxdir);
		if (max < 0) {
			kfree(params1);
			goto _end;
		}
		if (boundary_nearer(max, maxdir, best, valdir, min, 
mindir)) {
			*params = *params1;
			last = 1;
		}
		...
		*params = *save;
		max = snd_pcm_hw_param_max(pcm, params, var, max, 
&maxdir);
		if (max < 0) {
			kfree(save);
			return max;
	...
	if (hw_is_mask(var)) {
		...
		if (val == 0 && dir < 0) {
			...
			if (dir > 0)
				...
				val--;
			changed = 
snd_mask_refine_set(hw_param_mask(params, var), val);
		}
	} else if (hw_is_interval(var)) {
...
static long snd_pcm_oss_bytes(struct snd_pcm_substream *substream, long 
frames)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static long snd_pcm_alsa_frames(struct snd_pcm_substream *substream, long 
bytes)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	long buffer_size = snd_pcm_lib_buffer_bytes(substream);
...
oss_frame_size);
	...
	if (substream->oss.setup.period_size > 16)
		...
		int sd;
		size_t bytes_per_sec = params_rate(oss_params) * 
snd_pcm_format_physical_width(params_format(oss_params)) * 
params_channels(oss_params) / 8;
...
snd_pcm_hw_param_value_min(slave_params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, 
NULL));
...
snd_pcm_hw_param_value_max(slave_params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, 
NULL));
...
SNDRV_PCM_HW_PARAM_PERIODS, NULL);
	...
	s = snd_pcm_hw_param_value_min(slave_params, 
SNDRV_PCM_HW_PARAM_PERIODS, NULL);
	if (s < 2)
		s = 2;
	if (oss_periods < s)
...
static int choose_rate(struct snd_pcm_substream *substream,
		       struct snd_pcm_hw_params *params, unsigned int 
best_rate)
{
...
static void snd_pcm_oss_release_buffers(struct snd_pcm_substream 
*substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
int)SNDRV_PCM_ACCESS_RW_NONINTERLEAVED);
	...
	err = snd_pcm_hw_param_mask(substream, sparams, 
SNDRV_PCM_HW_PARAM_ACCESS, &mask);
	...
	sformat_mask = hw_param_mask_c(sparams, 
SNDRV_PCM_HW_PARAM_FORMAT);
 ...
 format_found:
	err = _snd_pcm_hw_param_set(sparams, SNDRV_PCM_HW_PARAM_FORMAT, 
(__force int)sformat, 0);
	...
	if (direct) {
		...
		_snd_pcm_hw_param_set(params, SNDRV_PCM_HW_PARAM_ACCESS,
				      (__force 
int)SNDRV_PCM_ACCESS_RW_INTERLEAVED, 0);
		_snd_pcm_hw_param_set(params, SNDRV_PCM_HW_PARAM_FORMAT,
				      (__force 
int)snd_pcm_oss_format_from(runtime->oss.format), 0);
		_snd_pcm_hw_param_set(params, SNDRV_PCM_HW_PARAM_CHANNELS,
				      runtime->oss.channels, 0);
		_snd_pcm_hw_param_set(params, SNDRV_PCM_HW_PARAM_RATE,
				      runtime->oss.rate, 0);
		pdprintf("client: access = %i, format = %i, channels = %i, 
rate = %i\n",
			 params_access(params), params_format(params),
			 params_channels(params), params_rate(params));
	}
	pdprintf("slave: access = %i, format = %i, channels = %i, rate = 
%i\n",
...
sparams);
	...
	if (!direct) {
		...
		err = snd_pcm_plug_format_plugins(substream, params, 
sparams);
		if (err < 0) {
			pcm_dbg(substream->pcm,
				"snd_pcm_plug_format_plugins failed: 
%i\n", err);
			goto failure;
		}
		if (runtime->oss.plugin_first) {
			struct snd_pcm_plugin *plugin;
			err = snd_pcm_plugin_build_io(substream, sparams, 
&plugin);
			if (err < 0) {
				pcm_dbg(substream->pcm,
					"snd_pcm_plugin_build_io failed: 
%i\n", err);
				goto failure;
			}
			if (substream->stream == 
SNDRV_PCM_STREAM_PLAYBACK) {
	...
	sw_params->avail_min = substream->stream == 
SNDRV_PCM_STREAM_PLAYBACK ?
...
params_period_size(sparams));
	...
	oss_buffer_size = array_size(oss_period_size, 
runtime->oss.periods);
	if (oss_buffer_size <= 0) {
		err = -EINVAL;
		goto failure;
...
snd_pcm_oss_format_to(params_format(params));
	...
	kvfree(runtime->oss.buffer);
	runtime->oss.buffer = kvzalloc(runtime->oss.period_bytes, 
GFP_KERNEL);
...
runtime->dma_area, bytes_to_samples(runtime, runtime->dma_bytes));

	runtime->oss.period_frames = snd_pcm_alsa_frames(substream, 
oss_period_size);
...
static int snd_pcm_oss_get_active_substream(struct snd_pcm_oss_file 
...
NULL);
	if (err < 0) {
		pcm_dbg(substream->pcm,
			"snd_pcm_oss_prepare: SNDRV_PCM_IOCTL_PREPARE 
failed\n");
		return err;
	}
...
/* call with params_lock held */
static int snd_pcm_oss_make_ready_locked(struct snd_pcm_substream 
*substream)
{
	struct snd_pcm_runtime *runtime;
	int err;
...
static int snd_pcm_oss_capture_position_fixup(struct snd_pcm_substream 
*substream, snd_pcm_sframes_t *delay)
{
	struct snd_pcm_runtime *runtime;
	snd_pcm_uframes_t frames;
	int err = 0;
...
SNDRV_PCM_IOCTL_DELAY, delay);
		if (err < 0)
			break;
		runtime = substream->runtime;
		if (*delay <= (snd_pcm_sframes_t)runtime->buffer_size)
			break;
		/* in case of overrun, skip whole periods like OSS/Linux 
driver does */
		/* until avail(delay) <= buffer_size */
		frames = (*delay - runtime->buffer_size) + 
runtime->period_size - 1;
		frames /= runtime->period_size;
		frames *= runtime->period_size;
		err = snd_pcm_kernel_ioctl(substream, 
SNDRV_PCM_IOCTL_FORWARD, &frames);
		if (err < 0)
			break;
	}
	return err;
...
const char *ptr, snd_pcm_uframes_t frames, int in_kernel)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	int ret;
	while (1) {
...
char *ptr, snd_pcm_uframes_t frames, int in_kernel)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static ssize_t snd_pcm_oss_write2(struct snd_pcm_substream *substream, 
const char *buf, size_t bytes, int in_kernel)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
(runtime->oss.plugin_first->src_width * 
runtime->oss.plugin_first->src_format.channels) / 8;
		if (!in_kernel) {
			if (copy_from_user(runtime->oss.buffer, (const 
char __force __user *)buf, bytes))
				return -EFAULT;
			buf = runtime->oss.buffer;
		}
		frames = bytes / oss_frame_bytes;
		frames1 = snd_pcm_plug_client_channels_buf(substream, 
(char *)buf, frames, &channels);
		if (frames1 < 0)
			return frames1;
		frames1 = snd_pcm_plug_write_transfer(substream, channels, 
frames1);
		if (frames1 <= 0)
			return frames1;
...
#endif
	{
		...
		frames1 = snd_pcm_oss_write3(substream, buf, frames, 
in_kernel);
		if (frames1 <= 0)
			return frames1;
...
static ssize_t snd_pcm_oss_write1(struct snd_pcm_substream *substream, 
...
static ssize_t snd_pcm_oss_read2(struct snd_pcm_substream *substream, char 
*buf, size_t bytes, int in_kernel)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	snd_pcm_sframes_t frames, frames1;
#ifdef CONFIG_SND_PCM_OSS_PLUGINS
	...
	if (runtime->oss.plugin_first) {
		struct snd_pcm_plugin_channel *channels;
		size_t oss_frame_bytes = 
(runtime->oss.plugin_last->dst_width * 
runtime->oss.plugin_last->dst_format.channels) / 8;
		if (!in_kernel)
			buf = runtime->oss.buffer;
		frames = bytes / oss_frame_bytes;
		frames1 = snd_pcm_plug_client_channels_buf(substream, buf, 
frames, &channels);
		if (frames1 < 0)
			return frames1;
		frames1 = snd_pcm_plug_read_transfer(substream, channels, 
frames1);
		if (frames1 <= 0)
			return frames1;
		bytes = frames1 * oss_frame_bytes;
	...
	{
		frames = bytes_to_frames(runtime, bytes);
		frames1 = snd_pcm_oss_read3(substream, buf, frames, 
in_kernel);
		if (frames1 <= 0)
			return frames1;
		bytes = frames_to_bytes(runtime, frames1);
...
(runtime->oss.period_ptr - runtime->oss.buffer_used), tmp)) {
			...
			tmp = snd_pcm_oss_read2(substream, (char __force 
*)buf,
						runtime->oss.period_bytes, 
0);
			if (tmp <= 0)
...
static int snd_pcm_oss_reset(struct snd_pcm_oss_file *pcm_oss_file)
{
	...
	for (i = 0; i < 2; i++) { 
		...
		if (!substream)
			continue;
		runtime = substream->runtime;
		snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DROP, 
NULL);
		mutex_lock(&runtime->oss.params_lock);
		runtime->oss.prepare = 1;
		runtime->oss.buffer_used = 0;
...
static int snd_pcm_oss_post(struct snd_pcm_oss_file *pcm_oss_file)
{
	...
	if (substream != NULL) {
		...
		if (err < 0)
			return err;
		snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_START, 
NULL);
	}
...
static int snd_pcm_oss_sync1(struct snd_pcm_substream *substream, size_t 
size)
{
	struct snd_pcm_runtime *runtime;
	ssize_t result = 0;
...
static int snd_pcm_oss_set_rate(struct snd_pcm_oss_file *pcm_oss_file, int 
rate)
{
	int idx;
...
static int snd_pcm_oss_set_channels(struct snd_pcm_oss_file *pcm_oss_file, 
...
static int snd_pcm_oss_get_block_size(struct snd_pcm_oss_file 
*pcm_oss_file)
{
	struct snd_pcm_substream *substream;
	int err;
...
static int snd_pcm_oss_get_formats(struct snd_pcm_oss_file *pcm_oss_file)
{
	...
	for (fmt = 0; fmt < 32; ++fmt) {
		if (snd_mask_test(format_mask, fmt)) {
			int f = snd_pcm_oss_format_to((__force 
...
static int snd_pcm_oss_set_format(struct snd_pcm_oss_file *pcm_oss_file, 
int format)
{
	int formats, idx;
	int err;
...
static int snd_pcm_oss_set_subdivide1(struct snd_pcm_substream *substream, 
int subdivide)
{
	struct snd_pcm_runtime *runtime;
...
static int snd_pcm_oss_set_subdivide(struct snd_pcm_oss_file 
*pcm_oss_file, int subdivide)
{
	int err = -EINVAL, idx;
...
static int snd_pcm_oss_set_fragment(struct snd_pcm_oss_file *pcm_oss_file, 
unsigned int val)
{
	int err = -EINVAL, idx;
...
static int snd_pcm_oss_get_caps1(struct snd_pcm_substream *substream, int 
res)
{
...
static int snd_pcm_oss_get_caps(struct snd_pcm_oss_file *pcm_oss_file)
{
	...
	result = DSP_CAP_TRIGGER | DSP_CAP_MMAP	| DSP_CAP_DUPLEX | 
DSP_CAP_REALTIME;
	for (idx = 0; idx < 2; idx++) {
		struct snd_pcm_substream *substream = 
pcm_oss_file->streams[idx];
...
static int snd_pcm_oss_set_trigger(struct snd_pcm_oss_file *pcm_oss_file, 
int trigger)
{
	struct snd_pcm_runtime *runtime;
...
psubstream->runtime->oss.trigger)
		result |= PCM_ENABLE_OUTPUT;
	if (csubstream && csubstream->runtime && 
csubstream->runtime->oss.trigger)
...
static int snd_pcm_oss_get_ptr(struct snd_pcm_oss_file *pcm_oss_file, int 
stream, struct count_info __user * _info)
{	
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
	snd_pcm_sframes_t delay;
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		err = snd_pcm_kernel_ioctl(substream, 
SNDRV_PCM_IOCTL_DELAY, &delay);
		if (err == -EPIPE || err == -ESTRPIPE || (! err && delay < 
0)) {
			err = 0;
			delay = 0;
			fixup = 0;
		...
		err = snd_pcm_oss_capture_position_fixup(substream, 
&delay);
		fixup = -runtime->oss.buffer_used;
	}
	if (err < 0)
...
runtime->buffer_size);
	if (atomic_read(&substream->mmap_count)) {
		...
		info.bytes = snd_pcm_oss_bytes(substream, 
runtime->status->hw_ptr) & INT_MAX;
	} else {
		delay = snd_pcm_oss_bytes(substream, delay);
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (substream->oss.setup.buggyptr)
				info.blocks = (runtime->oss.buffer_bytes - 
delay - fixup) / runtime->oss.period_bytes;
			else
				info.blocks = (delay + fixup) / 
runtime->oss.period_bytes;
...
static int snd_pcm_oss_get_space(struct snd_pcm_oss_file *pcm_oss_file, 
int stream, struct audio_buf_info __user *_info)
{
	struct snd_pcm_substream *substream;
	struct snd_pcm_runtime *runtime;
...
runtime->oss.periods;
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			err = snd_pcm_kernel_ioctl(substream, 
SNDRV_PCM_IOCTL_DELAY, &avail);
			if (err == -EPIPE || err == -ESTRPIPE || (! err && 
avail < 0)) {
			...
			err = 
snd_pcm_oss_capture_position_fixup(substream, &avail);
			fixup = runtime->oss.buffer_used;
		}
		if (err < 0)
			return err;
...
#ifdef OSS_DEBUG
	pcm_dbg(substream->pcm,
		"pcm_oss: space: bytes = %i, fragments = %i, fragstotal = 
%i, fragsize = %i\n",
		info.bytes, info.fragments, info.fragstotal, 
info.fragsize);
#endif
	if (copy_to_user(_info, &info, sizeof(info)))
		return -EFAULT;
...
static int snd_pcm_oss_get_mapbuf(struct snd_pcm_oss_file *pcm_oss_file, 
int stream, struct buffmem_desc __user * _info)
{
...
static void snd_pcm_oss_release_substream(struct snd_pcm_substream 
*substream)
{
	snd_pcm_oss_release_buffers(substream);
	substream->oss.oss = 0;
...
static void snd_pcm_oss_init_substream(struct snd_pcm_substream 
*substream,
				       struct snd_pcm_oss_setup *setup,
				       int minor)
{
				 ...
				 struct snd_pcm_oss_setup *setup)
{
	...
	if ((f_mode & (FMODE_WRITE|FMODE_READ)) == 
...
static int snd_task_name(struct task_struct *task, char *name, size_t 
size)
{
	unsigned int idx;
...
static long snd_pcm_oss_ioctl(struct file *file, unsigned int cmd, 
unsigned long arg)
{
	struct snd_pcm_oss_file *pcm_oss_file;
	...
	if (((cmd >> 8) & 0xff) == 'M')	{	/* mixer ioctl - for OSS 
compatibility */
		struct snd_pcm_substream *substream;
		int idx;
		for (idx = 0; idx < 2; ++idx) {
		...
		if (snd_BUG_ON(idx >= 2))
			return -ENXIO;
		return snd_mixer_oss_ioctl_card(substream->pcm->card, cmd, 
arg);
	}
#endif
	if (((cmd >> 8) & 0xff) != 'P')
...
SNDRV_PCM_STREAM_PLAYBACK,
	...
	case SNDCTL_DSP_GETODELAY:
		...
		if (res < 0) {
			/* it's for sure, some broken apps don't check for 
error codes */
			put_user(0, p);
			return res;
		}
...
{
	/*
	 * Everything is compatbile except 
SNDCTL_DSP_MAPINBUF/SNDCTL_DSP_MAPOUTBUF,
	 * which are not implemented for the native case either
	 */
	return snd_pcm_oss_ioctl(file, cmd, (unsigned 
long)compat_ptr(arg));
}
#else
...
static ssize_t snd_pcm_oss_read(struct file *file, char __user *buf, 
...
static ssize_t snd_pcm_oss_write(struct file *file, const char __user 
...
#ifdef OSS_DEBUG
	pcm_dbg(substream->pcm, "pcm_oss: write %li bytes (wrote %li 
bytes)\n",
...
static int snd_pcm_oss_mmap(struct file *file, struct vm_area_struct 
*area)
{
	struct snd_pcm_oss_file *pcm_oss_file;
	...
	case VM_READ | VM_WRITE:
		substream = 
pcm_oss_file->streams[SNDRV_PCM_STREAM_PLAYBACK];
		if (substream)
			break;
		fallthrough;
	case VM_READ:
...
pcm_oss_file->streams[SNDRV_PCM_STREAM_PLAYBACK];
	...
	if (runtime->oss.params) {
		/* use mutex_trylock() for params_lock for avoiding a 
deadlock
				   ...
				   struct snd_info_buffer *buffer)
{
	...
	while (!snd_info_get_line(buffer, line, sizeof(line))) {
		...
		ptr = snd_info_get_str(task_name, line, 
sizeof(task_name));
		if (!strcmp(task_name, "clear") || !strcmp(task_name, 
"erase")) {
			snd_pcm_oss_proc_free_setup_list(pstr);
			mutex_unlock(&pstr->oss.setup_mutex);
			continue;
		}
		for (setup = pstr->oss.setup_list; setup; setup = 
setup->next) {
			if (!strcmp(setup->task_name, task_name)) {
				template = *setup;
				break;
...
static void snd_pcm_oss_proc_init(struct snd_pcm *pcm)
{
	...
	for (stream = 0; stream < 2; ++stream) {
		...
		if (pstr->substream_count == 0)
			continue;
		entry = snd_info_create_card_entry(pcm->card, "oss", 
pstr->proc_root);
		if (entry) {
...
static int snd_pcm_oss_register_minor(struct snd_pcm *pcm)
{
	...
	if (dsp_map[pcm->card->number] == (int)pcm->device) {
		...
		duplex = 
(pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream_count > 0 && 
...
static int __init alsa_pcm_oss_init(void)
{
	...
	for (i = 0; i < SNDRV_CARDS; i++) {
		if (dsp_map[i] < 0 || dsp_map[i] >= SNDRV_PCM_DEVICES) {
			pr_err("ALSA: pcm_oss: invalid dsp_map[%d] = 
%d\n",
				   i, dsp_map[i]);
			dsp_map[i] = 0;
		}
		if (adsp_map[i] < 0 || adsp_map[i] >= SNDRV_PCM_DEVICES) {
			pr_err("ALSA: pcm_oss: invalid adsp_map[%d] = 
%d\n",
				   i, adsp_map[i]);
			adsp_map[i] = 1;
		}
