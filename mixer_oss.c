...
sizeof(info.id));
	strscpy(info.name, mixer && mixer->name[0] ? mixer->name : 
card->mixername, sizeof(info.name));
...
static int snd_mixer_oss_set_recsrc(struct snd_mixer_oss_file *fmixer, int 
recsrc)
{
	struct snd_mixer_oss *mixer = fmixer->mixer;
	struct snd_mixer_oss_slot *pslot;
...
static int snd_mixer_oss_get_volume(struct snd_mixer_oss_file *fmixer, int 
slot)
{
	struct snd_mixer_oss *mixer = fmixer->mixer;
	struct snd_mixer_oss_slot *pslot;
				    ...
				    int slot, int volume)
{
	struct snd_mixer_oss *mixer = fmixer->mixer;
	struct snd_mixer_oss_slot *pslot;
	int result = 0, left = volume & 0xff, right = (volume >> 8) & 
0xff;
...
static int snd_mixer_oss_ioctl1(struct snd_mixer_oss_file *fmixer, 
...
static long snd_mixer_oss_ioctl(struct file *file, unsigned int cmd, 
unsigned long arg)
{
...
#ifdef CONFIG_COMPAT
/* all compatible */
static long snd_mixer_oss_ioctl_compat(struct file *file, unsigned int 
cmd,
				       unsigned long arg)
{
...
static long snd_mixer_oss_conv(long val, long omin, long omax, long nmin, 
long nmax)
{
...
static struct snd_kcontrol *snd_mixer_oss_test_id(struct snd_mixer_oss 
*mixer, const char *name, int index)
{
					  ...
					  int *left, int *right)
{
	...
	if (uinfo->type == SNDRV_CTL_ELEM_TYPE_BOOLEAN &&
	    uinfo->value.integer.min == 0 && uinfo->value.integer.max == 
1)
		goto __unalloc;
	*left = snd_mixer_oss_conv1(uctl->value.integer.value[0], 
uinfo->value.integer.min, uinfo->value.integer.max, &pslot->volume[0]);
	if (uinfo->count > 1)
		*right = snd_mixer_oss_conv1(uctl->value.integer.value[1], 
uinfo->value.integer.min, uinfo->value.integer.max, &pslot->volume[1]);
      __unalloc:
	up_read(&card->controls_rwsem);
      	kfree(uctl);
      	kfree(uinfo);
...
static void snd_mixer_oss_get_volume1_sw(struct snd_mixer_oss_file 
*fmixer,
					 struct snd_mixer_oss_slot *pslot,
					 unsigned int numid,
					 int *left, int *right,
				     ...
				     int *left, int *right)
{
	...
	*left = *right = 100;
	if (slot->present & SNDRV_MIXER_OSS_PRESENT_PVOLUME) {
		snd_mixer_oss_get_volume1_vol(fmixer, pslot, 
...
slot->numid[SNDRV_MIXER_OSS_ITEM_GVOLUME], left, right);
		...
		snd_mixer_oss_get_volume1_vol(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_GLOBAL], left, right);
	}
	if (slot->present & SNDRV_MIXER_OSS_PRESENT_PSWITCH) {
		snd_mixer_oss_get_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_PSWITCH], left, right, 0);
	} else if (slot->present & SNDRV_MIXER_OSS_PRESENT_GSWITCH) {
		snd_mixer_oss_get_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_GSWITCH], left, right, 0);
	} else if (slot->present & SNDRV_MIXER_OSS_PRESENT_PROUTE) {
		snd_mixer_oss_get_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_PROUTE], left, right, 1);
	} else if (slot->present & SNDRV_MIXER_OSS_PRESENT_GROUTE) {
		snd_mixer_oss_get_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_GROUTE], left, right, 1);
	}
	return 0;
}

static void snd_mixer_oss_put_volume1_vol(struct snd_mixer_oss_file 
*fmixer,
					  struct snd_mixer_oss_slot 
*pslot,
					  unsigned int numid,
					 ...
					 int route)
{
	...
	if (uinfo->count > 1) {
		...
		uctl->value.integer.value[route ? 3 : 1] = right > 0 ? 1 : 
0;
		if (route) {
		...
		uctl->value.integer.value[0] = (left > 0 || right > 0) ? 1 
: 0;
	}
	res = kctl->put(kctl, uctl);
	if (res < 0)
...
slot->numid[SNDRV_MIXER_OSS_ITEM_CVOLUME], left, right);
		...
		snd_mixer_oss_put_volume1_vol(fmixer, pslot,
			slot->numid[SNDRV_MIXER_OSS_ITEM_CVOLUME], left, 
right);
	} else if (slot->present & SNDRV_MIXER_OSS_PRESENT_GVOLUME) {
		snd_mixer_oss_put_volume1_vol(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_GVOLUME], left, right);
...
slot->numid[SNDRV_MIXER_OSS_ITEM_GLOBAL], left, right);
	...
	if (left || right) {
		if (slot->present & SNDRV_MIXER_OSS_PRESENT_PSWITCH)
			snd_mixer_oss_put_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_PSWITCH], left, right, 0);
		if (slot->present & SNDRV_MIXER_OSS_PRESENT_CSWITCH)
			snd_mixer_oss_put_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_CSWITCH], left, right, 0);
...
slot->numid[SNDRV_MIXER_OSS_ITEM_PROUTE], left, right, 1);
		if (slot->present & SNDRV_MIXER_OSS_PRESENT_CROUTE)
			snd_mixer_oss_put_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_CROUTE], left, right, 1);
...
static int snd_mixer_oss_get_recsrc1_route(struct snd_mixer_oss_file 
*fmixer,
					   struct snd_mixer_oss_slot 
*pslot,
					   int *active)
					...
					int active)
{
	...
	snd_mixer_oss_put_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_CSWITCH], active, active, 0);
	return 0;
}
					   ...
					   int active)
{
	...
	snd_mixer_oss_put_volume1_sw(fmixer, pslot, 
slot->numid[SNDRV_MIXER_OSS_ITEM_CROUTE], active, active, 1);
	return 0;
}

static int snd_mixer_oss_get_recsrc2(struct snd_mixer_oss_file *fmixer, 
...
static int snd_mixer_oss_put_recsrc2(struct snd_mixer_oss_file *fmixer, 
unsigned int active_index)
{
	struct snd_card *card = fmixer->card;
	struct snd_mixer_oss *mixer = fmixer->mixer;
	...
	if (err > 0)
		snd_ctl_notify(fmixer->card, SNDRV_CTL_EVENT_MASK_VALUE, 
&kctl->id);
	err = 0;
      __unlock:
	up_read(&card->controls_rwsem);
...
static int snd_mixer_oss_build_test(struct snd_mixer_oss *mixer, struct 
slot *slot, const char *name, int index, int item)
{
...
static int snd_mixer_oss_build_test_all(struct snd_mixer_oss *mixer,
					const struct 
snd_mixer_oss_assign_table *ptr,
					struct slot *slot)
{
	char str[64];
	int err;
				     ...
				     int ptr_allocated, int replace_old)
{
	...
	if (kctl) {
		...
		if (!strcmp(uinfo->value.enumerated.name, str)) {
			...
			for (slot.capture_item = 1; slot.capture_item < 
uinfo->value.enumerated.items; slot.capture_item++) {
...
up_read(&mixer->card->controls_rwsem);
				...
				if (!strcmp(uinfo->value.enumerated.name, 
str)) {
					slot.present |= 
SNDRV_MIXER_OSS_PRESENT_CAPTURE;
	...
	if (slot.present != 0) {
		...
		*pslot = slot;
		...
		/* note: ES18xx have both Capture Source and XX Capture 
Volume !!! */
		if (slot.present & SNDRV_MIXER_OSS_PRESENT_CSWITCH) {
				     ...
				     struct snd_info_buffer *buffer)
{
	...
	while (!snd_info_get_line(buffer, line, sizeof(line))) {
		...
		for (ch = 0; ch < SNDRV_OSS_MAX_MIXERS; ch++)
			if (oss_mixer_names[ch] && 
strcmp(oss_mixer_names[ch], str) == 0)
				break;
		if (ch >= SNDRV_OSS_MAX_MIXERS) {
			pr_err("ALSA: mixer_oss: invalid OSS volume 
'%s'\n",
			       str);
			continue;
		}
...
idx);
		...
		if (slot && slot->assigned &&
		    slot->assigned->index == idx && ! 
strcmp(slot->assigned->name, str))
			/* not changed */
			goto __unlock;
...
static int snd_mixer_oss_notify_handler(struct snd_card *card, int cmd)
{
	...
	if (cmd == SND_MIXER_OSS_NOTIFY_REGISTER) {
		...
		if (err < 0) {
			dev_err(card->dev,
				"unable to register OSS mixer device 
%i:%i\n",
				card->number, 0);
			kfree(mixer);
			return err;
...
static int __init alsa_mixer_oss_init(void)
{
	...
	for (idx = 0; idx < SNDRV_CARDS; idx++) {
		...
		if (card) {
			snd_mixer_oss_notify_handler(card, 
